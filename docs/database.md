...
- `employees`: Factory hosts with employee codes, phone numbers, and optional linked system user accounts.
...
- `visits`: Transactional records of each visit event with statuses:
  - `REGISTERED`
  - `PRE_REGISTERED`
  - `PENDING_APPROVAL`
  - `APPROVED`
  ...
  - `CHECKED_OUT`
  - `CANCELLED`
- `visitor_passes`: Security badge metadata containing unique pass numbers (`PASS-BGP-0001`), high-entropy QR tokens (`qr_<hex>`), validity timestamps, and print counters.
...
- `audit_logs`: Immutable security ledger tracking all system logins, status changes, pass reprints, and profile modifications with IP addresses and user agents.
...
| `idx_visitor_passes_qr` | `visitor_passes` | `(qr_token)` | Instant $O(1)$ gate QR scanner lookup |
